...
public class CsrfHeaderFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (cookie == null || csrfTokenValue == null || !csrfTokenValue.equals(cookie.getValue())) {
			accessDeniedHandler.handle(request, response,
					new AccessDeniedException("Missing or non-matching XSRF-TOKEN"));
			return;
		}
	...
	protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
		return allowedMethods.matcher(request.getMethod()).matches()
				|| request.getServletPath().startsWith(Catalago.URL_PUBLIC_BASE);
	}
	...
	public static void addCsrfCookie(HttpServletResponse response) {
		...
		cookie.setPath(Catalago.URL_BASE);
